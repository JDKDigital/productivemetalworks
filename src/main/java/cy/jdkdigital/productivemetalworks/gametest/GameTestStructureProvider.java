...
/**
 * Generates the empty NBT structure template used as the gametest playground.
 *
 * <p>The vanilla {@code minecraft:empty} template is 1×1×1 — too small for a foundry
 * multiblock — so we ship a 7×7×7 variant and reference it from {@link MetalworksGameTests}.
 * The contents are deliberately empty: the test body places every block it needs at runtime
 * via {@code helper.setBlock}.
 */
public class GameTestStructureProvider implements DataProvider
{
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        Path path = pathProvider.file(Identifier.fromNamespaceAndPath(ProductiveMetalworks.MODID, NAME), "nbt");
    ...
    public String getName() {
        return "ProductiveMetalworks GameTest Structures";
    }
}
