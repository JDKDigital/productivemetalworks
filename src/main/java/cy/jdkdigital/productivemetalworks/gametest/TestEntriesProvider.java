 ...
 * <ul>
 *   <li>{@code data/productivemetalworks/test_environment/<test>_env.json} — a vanilla empty
 *       {@code all_of} environment. Required because every {@code FunctionGameTestInstance}
 *       must reference an environment Holder.</li>
 *   <li>{@code data/productivemetalworks/test_instance/<test>.json} — a vanilla
 *       {@code minecraft:function} test pointing at a {@code productivemetalworks:<test>}
...
public class TestEntriesProvider implements DataProvider
{
    ...
    public CompletableFuture<?> run(CachedOutput cache) {
        // Force <clinit> of MetalworksGameTests so register(...) calls populate MAX_TICKS.
        ...
        for (Map.Entry<String, Integer> entry : MetalworksGameTests.MAX_TICKS.entrySet()) {
            ...
            Identifier envId = Identifier.fromNamespaceAndPath(ProductiveMetalworks.MODID, name + "_env");
            Identifier instanceId = Identifier.fromNamespaceAndPath(ProductiveMetalworks.MODID, name);
    ...
    private static JsonObject instanceJson(String name, Identifier envId, int maxTicks) {
        ...
        obj.addProperty("function", Identifier.fromNamespaceAndPath(ProductiveMetalworks.MODID, name).toString());
        obj.addProperty("structure", Identifier.fromNamespaceAndPath(ProductiveMetalworks.MODID, "empty_7x7").toString());
    ...
    public String getName() {
        return "ProductiveMetalworks GameTest Entries";
    }
}
